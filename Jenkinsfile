          ...
          if (params.ACTION == 'apply') {
            ...
            dir("${BASE_DIR}/02-services") {
              ...
              sh """
                ...
                # fixed). Terraform's state has no record of any of these, so
                # a fresh `helm install` collides with the leftover release
                # object. Scoped to only releases confirmed failing —
                # argocd-root-app hasn't been reported broken, and deleting a
                # HEALTHY release's Helm record (unlike a failed one) would
                # strand its live resources unmanaged and likely cause
                # "already exists" conflicts on the next install, which is
                # worse than this. Remove this block after a run completes
